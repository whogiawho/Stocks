...
public class SimAmVolRDeltaHelper {
    public static void get(String args[]) {
        ...
        String sAmVolRDeltaFile = CmdLineUtils.getString(cmd, "f", null);
        String sDir = CmdLineUtils.getString(cmd, "d", null);
        if(sAmVolRDeltaFile==null||sDir==null)
            usage();
        //System.out.format("sAmVolRDeltaFile=%s sDir=%s\n", sAmVolRDeltaFile, sDir);
        ...
        if(tsTriple!=null) {
            ...
            //get amvolr delta for all sds of tradeDate
            handleAll(cmd);
        }
    }
    ...
    private static void handleSingle(String stockCode, String tradeDate, String hms, CommandLine cmd) {
        ...
        String sAmVolRDeltaFile = CmdLineUtils.getString(cmd, "f", null);
        String sDir = CmdLineUtils.getString(cmd, "d", null);
        String sAmVolR0 = sDir+"\\"+tradeDate+"."+hms+".txt";
        double threshold = AmVolRUtils.getThreshold(cmd);

        ArrayList<DeltaSimRecord> avrrList = DeltaSimRecord.getList(sAmVolRDeltaFile);
        //System.out.format("avrrList.size=%d\n", avrrList.size());


        ColLoader cl = new ColLoader();
        ArrayList<String> sList0 = new ArrayList<String>();
        cl.load(sList0, sAmVolR0, 1);
        ...
        for(int i=0; i<avrrList.size(); i++) {
            ...
            String sAmVolR1 = sDir+"\\"+r.tradeDate+"."+r.hms+".txt";
            sList1.clear();
            cl.load(sList1, sAmVolR1, 1);
    ...
    private static double[][] getAmVolRDeltaMatrix(ArrayList<DeltaSimRecord> aarList, String sDir) {
        int cols = aarList.size();

        DeltaSimRecord r = aarList.get(0);
        String sAmVolR = sDir+"\\"+r.tradeDate+"."+r.hms+".txt";
        ColLoader cl = new ColLoader();
        ArrayList<String> sList0 = new ArrayList<String>();
        cl.load(sList0, sAmVolR, 1);
        int rows = sList0.size();

        double[][] m = new double[rows][cols];
        for(int i=0; i<cols; i++) {
            r = aarList.get(i);
            sAmVolR = sDir+"\\"+r.tradeDate+"."+r.hms+".txt";
            sList0.clear();
            cl.load(sList0, sAmVolR, 1);
    ...
    private static void handleAll(CommandLine cmd) {
        ...
        ArrayList<DeltaSimRecord> aarList = DeltaSimRecord.getList(sAmVolRDeltaFile);
        double[][] m = getAmVolRDeltaMatrix(aarList, sDir);
    ...
    private static void usage() {
        System.err.println("usage: java AnalyzeTools simamvolrdelta [-fdh] [stockCode,tradeDate,hms]");
        System.err.println("       loop amvolrdeltaFile to get correls with each other or");
        ...
        System.err.println("       -f amvolrdeltaFile  ; the file generated from amvolrdelta");
        System.err.println("       -d dir              ; the dir where the amvolr files of amvolrdelta are");
    ...
    public static Options getOptions() {
        ...
        options.addOption("f", true,  "the file generated from amvolrdelta");
        options.addOption("d", true,  "the dir where the amvolrdelta files are");
